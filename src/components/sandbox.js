...
function createScene() {
	...
	glitch = new GlitchPass(10);
	unreal = new UnrealBloomPass(
		{ x: window.innerWidth, y: window.innerHeight },
		2.0,
		0.4,
		0.375
	);
	film = new FilmPass(0.8, 0.325, 300, false);
...
function createGUI() {
	...
	postProcess.add(glitch, "enabled").listen().name("GlitchPass");
	postProcess.add(unreal, "enabled").listen().name("UnrealBloomPass");
	postProcess.add(film, "enabled").listen().name("FilmPass");
	postProcess.open();

	const cameraFolder = gui.addFolder("Camera Position");
	cameraFolder.add(camera.position, "x", 0.0, 30.0).step(0.001);
	cameraFolder.add(camera.position, "y", 0.0, 30.0).step(0.001);
	cameraFolder.add(camera.position, "z", 0.0, 30.0).step(0.001);
	// cameraFolder.open();
	...
	const primitivePosGUI = gui.addFolder("Perlin Noise Position");
	primitivePosGUI.add(_primitive.mesh.position, "x", 0.0, 10.0).step(0.001);
	primitivePosGUI.add(_primitive.mesh.position, "y", 0.0, 10.0).step(0.001);
	primitivePosGUI.add(_primitive.mesh.position, "z", 0.0, 10.0).step(0.001);
	// primitivePosGUI.open();

	const statueGUI = gui.addFolder("Statue Position");
